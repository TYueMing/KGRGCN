...
def main():
    ...
    try:
        ...
        walker_spawn_point = carla.Transform(
            carla.Location(
                x=new_location.x + forward_vector.x * 110.0,
                y=new_location.y + forward_vector.x * 3 ,
                z=new_location.z + forward_vector.x * 0.1
        ...
        try:
            ...
            video_fps = 60
            video_save_path = 'result/test_ghost_1-19.mp4'
            ...
            while True:
                ...
                try:
                    ...
                    #####  调用TKG生成GCN训练数据
                    if len(traffic_participants_list) >= 10 and frame_total % space_frame ==1 and frame_total>=300:
                        scene_final = scene
            ...
            if predicted_index == 1:
                ...
                # 等待键盘事件，按任意键关闭窗口
                cv2.imwrite('result/ghost_result-1-19.jpg', final_img_all)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
            ...
            plt.axvline(x=dangerous_first_frame, color='red', linestyle='--',
                        label=f"Detect danger at frame x={dangerous_first_frame}")
            ...
            # 保存图像到本地
            plt.savefig('result/line_plot_ghost-1-19.png')
