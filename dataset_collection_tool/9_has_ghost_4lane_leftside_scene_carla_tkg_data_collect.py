...
def main():
    ...
    try:
        ...
        world.set_weather(preset[3][0])

        ##########   天气的内容
        weather_list = (
            "weather-attr", preset[3][1], ("w_direction", "w_level")
        )
        ...
        spawn_point = spawn_points[299] if len(spawn_points) > 300 else spawn_points[0]
        # 生成车辆
        ...
        spawn_point_front = spawn_points[298] if len(spawn_points) > 300 else spawn_points[0]
        # 计算新的位置，前方移动10米
        forward_vector = spawn_point_front.get_forward_vector()  # 获取生成点的前方向量
        new_location = spawn_point_front.location + forward_vector * 10.0  # 向前移动80米
        ...
        ######################### 创建车流 ##########################
        # # other-1
        other_vehicle_bp = blueprint_library.filter('vehicle.*')[0]
        spawn_point_other = spawn_points[300] if len(spawn_points) > 300 else spawn_points[0]
        # 计算新的位置，前方移动10米
        forward_distence = spawn_point_other.get_forward_vector()  # 获取生成点的前方向量
        new_location_other = spawn_point_other.location + forward_distence * 15.0  # 向前移动80米
        ...
        spawn_point_other_2 = spawn_points[301] if len(spawn_points) > 300 else spawn_points[0]
        # 计算新的位置，前方移动10米
        forward_distence_2 = spawn_point_other_2.get_forward_vector()  # 获取生成点的前方向量
        new_location_other_2 = spawn_point_other_2.location + forward_distence_2 * 25.0  # 向前移动80米
        ...
        #
        # # # other-3
        # other_vehicle_bp_3 = blueprint_library.filter('vehicle.*')[2]
        ...
        # new_location_other_3 = spawn_point_other_3.location - forward_distence_3 * 20.0  # 向前移动80米
        ...
        # new_location_other_4 = spawn_point_other_4.location - forward_distence_4 * 16.0  # 向前移动80米
        ...
        walker_spawn_point = carla.Transform(
            carla.Location(
                x=new_location.x + forward_vector.x * 90.0,
                y=new_location.y - forward_vector.x * 2.5 ,
                z=new_location.z
        ...
        try:
            ...
            while True:
                ...
                # # 实时计算他车与自车之间的距离
                # if ego_vehicle and front_vehicle:
                ...
                #         x=front_location.x - ego_location.x,
                #         y=front_location.y - ego_location.y,
                #         z=0  # 仅考虑平面上的夹角
                #     )
                ...
                try:
                    ...
                    for vehicle in vehicles:
                        ...
                        if horizontal_distance >=6:
                            lane = "lane_4"
                        else:
                            if horizontal_distance <= -2.5:
                                lane = "lane_1"
                            elif  -2.5< horizontal_distance <= 2:
                                lane = "lane_2"
                            else:
                                lane = "lane_3"
                    ...
                    if frame_total % space_frame ==1:    ################ ############  每10帧增加一个关键帧
                        if len(traffic_participants_list)<10:
                            ...
                            ego_information_list = ego_information_list + ( (("ego","vehicle","lane_2","speed",(speed_ego,"height","width")),), )
            ...
            with open(file_path + 'tkgcn_dataset/data' + str(txt_number) + '.txt', 'w') as f:
                ...
                # 添加空行作为分隔符
                f.write('\n\n')
                np.savetxt(f, dangerous_type[1], fmt='%f')
