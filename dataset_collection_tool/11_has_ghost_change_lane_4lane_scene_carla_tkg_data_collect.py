...
def main():
    ...
    try:
        ...
        # spawn_point_other_4 = spawn_points[299] if len(spawn_points) > 300 else spawn_points[0]
        # # 计算新的位置，前方移动10米
        # forward_distence_4 = spawn_point_other_4.get_forward_vector()  # 获取生成点的前方向量
        # new_location_other_4 = spawn_point_other_4.location - forward_distence_4 * 30.0  # 向前移动80米
        ...
        walker_spawn_point = carla.Transform(
            carla.Location(
                ...
                y=new_location.y + forward_vector.x * 0.2 ,
                z=new_location.z
        ...
        try:
            ...
            while True:
                ...
                if walker and front_vehicle:
                    ...
                    if distance<=25 and dis_y <=3 :
                        front_control.brake = 0.95
                        front_vehicle.apply_control(front_control)
                        if distance <=13 and dis_y <1.5 :
                            front_control.throttle = 0.7
                            front_control.steer = -0.2
                            front_vehicle.apply_control(front_control)
                        if distance <= 13 and dis_y >= 2:
                            front_control.steer = 0.2
