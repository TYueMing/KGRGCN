...
class TKGCN_V9_m(nn.Module):
    def __init__(self,
                 ...
                 num_features,
                 num_relations):
        super(TKGCN_V9_m, self).__init__()
    ...
    def forward(self, x, edge_index, edge_attr,batch=None):
        ...
        x_main_lane = F.dropout(x_main_lane, p=self.dropout_ratio, training=self.training)
...
# 创建模型实例
model = TKGCN_V9_m(nhid=64,pooling_rate=0.001,dropout_rate=0.01,num_classes=2,num_features=60,num_relations=75)

# 获取模型参数数量
total_params = count_parameters(model)
print(f"Total number of parameters: {total_params}")   # 3005315
